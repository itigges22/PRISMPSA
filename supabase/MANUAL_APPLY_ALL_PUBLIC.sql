...
CREATE OR REPLACE FUNCTION public.get_user_role_ids()
RETURNS UUID[] AS $$
BEGIN
  RETURN ARRAY(
    SELECT role_id 
...
BEGIN
  ...
  IF public.user_has_permission('view_all_projects') THEN
    RETURN TRUE;
  END IF;
  
  -- Must have base VIEW_PROJECTS permission
  IF NOT public.user_has_permission('view_projects') THEN
    RETURN FALSE;
  END IF;
  
  -- Is assigned to the project
  IF public.user_assigned_to_project(project_id) THEN
    RETURN TRUE;
  END IF;
...
BEGIN
  ...
  IF public.user_has_permission('edit_all_projects') THEN
    RETURN TRUE;
  END IF;
  
  -- Must have base EDIT_PROJECT permission
  IF NOT public.user_has_permission('edit_project') THEN
    RETURN FALSE;
  END IF;
  
  -- Is assigned to the project
  RETURN public.user_assigned_to_project(project_id);
END;
$$ LANGUAGE plpgsql SECURITY DEFINER STABLE;
...
BEGIN
  ...
  IF public.user_has_permission('delete_all_projects') THEN
    RETURN TRUE;
  END IF;
  
  -- Must have base DELETE_PROJECT permission
  IF NOT public.user_has_permission('delete_project') THEN
    RETURN FALSE;
  END IF;
...
-- Grant permissions
GRANT EXECUTE ON FUNCTION public.get_user_role_ids() TO authenticated;
...
GRANT EXECUTE ON FUNCTION public.get_user_department_ids() TO authenticated;
...
CREATE POLICY "roles_delete_policy" ON roles
  FOR DELETE
  USING (
    public.user_has_permission('delete_role')
    AND NOT is_system_role
  );
...
CREATE POLICY "user_roles_insert_policy" ON user_roles
  FOR INSERT
  WITH CHECK (public.user_has_permission('assign_users_to_roles'));

DROP POLICY IF EXISTS "user_roles_delete_policy" ON user_roles;
CREATE POLICY "user_roles_delete_policy" ON user_roles
  FOR DELETE
  USING (public.user_has_permission('remove_users_from_roles'));
...
CREATE POLICY "accounts_delete_policy" ON accounts
  FOR DELETE
  USING (public.user_has_permission('delete_account'));
...
CREATE POLICY "projects_insert_policy" ON projects
  FOR INSERT
  WITH CHECK (public.user_has_permission('create_project'));
...
CREATE POLICY "project_assignments_insert_policy" ON project_assignments
  FOR INSERT
  WITH CHECK (
    public.user_has_permission('assign_project_users')
...
CREATE POLICY "project_assignments_delete_policy" ON project_assignments
  FOR DELETE
  USING (
    public.user_has_permission('remove_project_users')
    AND public.can_edit_project(project_id)
  );
...
CREATE POLICY "tasks_select_policy" ON tasks
  FOR SELECT
  USING (
    public.user_has_permission('view_tasks')
...
CREATE POLICY "tasks_insert_policy" ON tasks
  FOR INSERT
  WITH CHECK (
    public.user_has_permission('create_task')
    AND public.can_edit_project(project_id)
  );
...
CREATE POLICY "tasks_update_policy" ON tasks
  FOR UPDATE
  USING (
    public.user_has_permission('edit_task')
    AND public.can_edit_project(project_id)
  );
...
CREATE POLICY "tasks_delete_policy" ON tasks
  FOR DELETE
  USING (
    public.user_has_permission('delete_task')
    AND public.can_edit_project(project_id)
  );
...
CREATE POLICY "deliverables_select_policy" ON deliverables
  FOR SELECT
  USING (
    public.user_has_permission('view_deliverables')
    AND public.can_view_project(project_id)
  );
...
CREATE POLICY "deliverables_insert_policy" ON deliverables
  FOR INSERT
  WITH CHECK (
    public.user_has_permission('upload_deliverable')
    AND public.can_edit_project(project_id)
  );
...
CREATE POLICY "deliverables_update_policy" ON deliverables
  FOR UPDATE
  USING (
    (public.user_has_permission('edit_deliverable') OR public.user_has_permission('approve_content'))
    AND public.can_edit_project(project_id)
  );
...
CREATE POLICY "deliverables_delete_policy" ON deliverables
  FOR DELETE
  USING (
    public.user_has_permission('delete_deliverable')
    AND public.can_edit_project(project_id)
  );
...
-- ================================================================================
-- SELECT * FROM public.user_is_superadmin();
-- SELECT * FROM public.user_has_permission('create_project');
