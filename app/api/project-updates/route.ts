...
export async function GET(request: NextRequest) {
  try {
    ...
    // If user has VIEW_ACCOUNT_PROJECTS_UPDATES, filter by accounts they're assigned to
      ...
      // Get accounts where user is assigned (through projects or as account manager)
      const { data: userAccounts } = await supabase
        .from('accounts')
        .select('id')
        .eq('account_manager_id', userId);

      // Get accounts from projects user created or is assigned to
      const { data: userProjects } = await supabase
        .from('projects')
        .select('account_id')
        .or(`created_by.eq.${userId},assigned_user_id.eq.${userId}`);

      // Also check project_assignments table
    ...
    // If user has VIEW_DEPARTMENT_PROJECTS_UPDATES, filter by department projects
      ...
      if (userDepartmentIds.length > 0) {
        ...
        const { data: departmentProjects } = await supabase
          .from('project_departments')
          .select('project_id')
          .in('department_id', userDepartmentIds);

        const projectIds = departmentProjects?.map((pd: any) => pd.project_id) || [];
    ...
    // If user has VIEW_ASSIGNED_PROJECTS_UPDATES, filter by assigned projects only
      ...
      // Get projects user is assigned to (via project_assignments or direct assignment)
